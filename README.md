...
La plupart du temps, j'utilise simplement des classes délimitées par des traits
d'union (ex. `. Foo-bar`, pas `. foo_bar` ou `. FooBar`), mais dans certaines
circonstances, j'utilise la notation BEM (Bloc, Élément, Modifieur).
...
dans le cas où vous savez que la règle sera **toujours ** prioritaire, par exemple `.error {color: red !important;}`.
